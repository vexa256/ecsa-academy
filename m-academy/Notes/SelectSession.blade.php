<div class="container px-5 py-5">
    ...
    <div class="card-body shadow-lg pt-3 bg-light table-responsive px-5 py-5">
        <div class="row">
            <div class="col-md-12">
                <form action="{{ route('MgtSessionNotes') }}" method="GET">
                    @csrf
                    <div class="mb-3 col-md-12  py-5   my-5">
                        <label id="label" for="" class="px-5   my-5 required form-label">Select
                            module session to attach notes to
                        </label>
                        <select required name="SID" class="form-select  py-5   my-5 form-select-solid" data-control="select2" data-placeholder="Select a option">
                            <option></option>
                            @isset($DatabaseData)

                            @foreach ($DatabaseData as $data)
                            <option value="{{ $data->SID }}">
                                {{ $data->SessionName }}</option>
                            @endforeach
                            @endisset
