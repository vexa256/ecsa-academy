...
// app/Helpers/InputTypeValidation.php
    ...
    public function MassInsert(Request $request)
    {
        ...
        foreach ($tableColumns as $column) {
            if ($request->hasFile($column)) {
                $rules[$column] = 'file|mimes:pdf,doc,webp,docx,jpg,jpeg,png|max:80000';
            } else {
    ...
    public function MassUpdate(Request $request)
    {
        ...
        try {
            ...
            // Handle file paths to match the format of MassInsert
            foreach ($uploadedFiles as $key => $value) {
                // Check if the file path is in the desired format
                if (strpos($updateData[$key], 'assets/docs') === false) {
                    // If not, update the file path to match the format
                    $updateData[$key] = 'assets/docs/' . basename($value);
                }
            }
            ...
            dd($e); // Debugging: Print the exception for debugging
